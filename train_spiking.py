...
# import the spiking neural network
...
def preprocess_state(state):
    ...
    return np.where(flattened > 0.5, 1.0, 0.0)  # if it's bright, spike, otherwise, 0.
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        net = SpikingNetwork(genome, config)                    # spiking neat
        ...
        while not done:
            ...
            if info['flag_get']:
                ...
                genome.fitness += 100000  # beats the 50k threshold immediately
...
if __name__ == "__main__":
    ...
    #config_path = os.path.join(local_dir, "config-MarI-O_NEAT.txt")    # standard neat
    #config_path = os.path.join(local_dir, "config-spiking.txt")         # spiking neat
    #run(config_path)
